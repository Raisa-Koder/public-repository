...
echo  "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON teams.team_id = games.winner_id FULL JOIN games AS g ON teams.team_id = g.opponent_id WHERE (g.year=2014 AND g.round='Eighth-Final') OR (games.year=2014 AND games.round='Eighth-Final')")"
